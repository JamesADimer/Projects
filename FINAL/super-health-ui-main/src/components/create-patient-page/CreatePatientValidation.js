const validate = (patient) => {
  const errors = {};
  if (!patient.firstName.toString().trim()) {
    errors.firstName = 'This field is required.';
  } else if (/^\s+|\s{2,}|\s+$/.test(patient.firstName)) {
    errors.firstName = 'First name must not contain multiple spaces in a row';
  } else if (/[^A-Za-z \-']/.test(patient.firstName)) {
    errors.firstName = 'Apostrophe ( \' ) and dash ( - ) are the only special characters allowed.';
  }
  if (!patient.lastName.toString().trim()) {
    errors.lastName = 'This field is required.';
  } else if (/^\s+|\s{2,}|\s+$/.test(patient.lastName)) {
    errors.lastName = 'Last name must not contain multiple spaces in a row';
  } else if (/[^A-Za-z \-']/.test(patient.lastName)) {
    errors.lastName = 'Apostrophe ( \' ) and dash ( - ) are the only special characters allowed.';
  }
